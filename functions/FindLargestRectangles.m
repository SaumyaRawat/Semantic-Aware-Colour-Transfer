...
%function[acc_matrix] = find_max_sky(im)
	%im = 1-im;                      %// convert "empty" regions to 0 intensity
%	im = imresize(im,[21,21]);
%	[rows,cols] = size(im);
...
%		for j = 2:cols
%	        if im(i,j) == 0
%	            acc_matrix(i,j) = acc_matrix(i,j-1)+1;
%	        else
%	            acc_matrix(i,j) = 0;
%	        end
%	    end
%	end
%	max_val = max(acc_matrix(:));
%	[col,row,~]=find(acc_matrix==max_val)
%
	%params = [ULrow ULcol BRrow BRcol];
	%area = (BRrow-ULrow)*(BRcol-ULcol); 
	%max_sky_region = im;
	%max_sky_region(params(1):params(3),params(2):params(4),1) = 0;
	%max_sky_region(params(1):params(3),params(2):params(4),2) = 255;
	%max_sky_region(params(1):params(3),params(2):params(4),3) = 255;
%end
